...
Graphics$Germ_lnConfCases <- ggplot() +
  geom_line(aes(x = index(Germany_ts$ConvCases), y = log(Germany_ts$ConvCases))) +
  scale_x_date(minor_breaks = function(x) seq.Date(from = min(x), to = max(x), by = "days"), breaks = function(x) seq.Date(from = min(x), to = max(x), by = "14 days")) +
  labs(title = "Germany ln of confirmed Corona cases",  x = "Time", y = "ln(Confirmed Cases)")
Graphics$Germ_lnConfCases
...
lw_model_exp <- lm(log(Data) ~ Index6, data = window(Germany_ts_3pModel, start = "2020-03-28"))
lw_model_lin <- lm(Data ~ Index6, data = window(Germany_ts_3pModel, start = "2020-03-28"))
...
ggplot() +
  ...
  geom_line(aes(x = index(window(Germany_ts_3pModel$Data, start = "2020-03-28")), y = exp(fitted(lw_model_exp)), color = "2")) +
  geom_line(aes(x = index(window(Germany_ts_3pModel$Data, start = "2020-03-28")), y = fitted(lw_model_lin), color = "3")) +
  coord_cartesian(xlim = range(time(window(Germany_ts_3pModel$Data, start = c("2020-03-28"))))) +
  scale_x_date(minor_breaks = function(x) seq.Date(from = min(x), to = max(x), by = "days"), breaks = function(x) seq.Date(from = min(x), to = max(x), by = "14 days")) +
  labs(title = "Germany confirmed Corona cases",  x = "Time", y = "Confirmed Cases") +
